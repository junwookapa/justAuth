...
public class UserServiceImpl implements UserService {
	...
	public UserStatus createUser(String body) {
		...
		if(mCollection.findOne(new BasicDBObject("userID", user.getUserID())) == null){
			...
			.append("password", mAESSecurity.encoding(user.getPassword()))
			.append("role", user.getRole())
			.append("email", user.getEmail())
			.append("affiliation", user.getAffiliation())
	...
	public User findbyUserID(String userID) {
		...
		try{
			return new User((BasicDBObject) mCollection.findOne(new BasicDBObject("userID", userID)));
	...
	public UserStatus login(String body) {
		...
		try{
			...
			compareUser = findbyUserID(user.getUserID());
